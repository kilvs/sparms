...
16. **Sparms uses section groups for header + footer.** `theme.liquid` uses `{% sections 'header-group' %}` (plural) and `{% sections 'footer-group' %}`, wrapped in `<div id="header-group">` / `<div id="footer-group">` (Dawn / Horizon pattern). The `sections/header-group.json` and `sections/footer-group.json` manifests reference `sections/header.liquid` and `sections/footer.liquid`. Don't switch to direct `{% section %}` (singular) calls — we tried that briefly and the merchant couldn't see Header / Footer settings in the theme editor sidebar (static sections don't reliably surface in the editor). See the section setup memory for full rationale. Critical distinction: `{% sections %}` (plural) loads a section group manifest `.json`; `{% section %}` (singular) loads a section file `.liquid` but loses editor visibility.
17. **Sticky navbar requires the `theme-shopify-fixes.css` override.** Webflow's `.component_navbar_section { position: sticky; top: 0 }` doesn't work on its own because Shopify wraps the section in a `<header class="shopify-section shopify-section-header">` that's the same height as the navbar — the sticky child has no scroll room within its parent. The fix in `assets/theme-shopify-fixes.css` promotes sticky to the wrapper class. Don't remove it. If sticky stops working, verify (a) the file is loaded from theme.liquid, (b) the header schema still has `"class": "shopify-section-header"` so the selector matches.
